...
func TestMemoryQueueBasicOperations(t *testing.T) {
	...
	// 测试空队列
	assert.Equal(t, 0, memQueue.Count(queueName), "空队列数量应该为0")
	assert.Empty(t, memQueue.Pop(queueName, 10), "从空队列获取项目应该返回空列表")
	assert.Empty(t, memQueue.List(queueName, 1, 10), "列出空队列应该返回空列表")
	...
	err := memQueue.Add(queueName, testItem)
	assert.NoError(t, err, "添加队列项应该成功")

	// 测试计数和列表
	assert.Equal(t, 1, memQueue.Count(queueName), "队列应该有1个项目")
	items := memQueue.List(queueName, 1, 10)
	assert.Len(t, items, 1, "应该列出1个项目")
	assert.Equal(t, testItem.ID, items[0].ID, "列出的项目ID应该匹配")

	// 测试弹出
	poppedItems := memQueue.Pop(queueName, 10)
	assert.Len(t, poppedItems, 1, "应该获取1个项目")
	assert.Equal(t, testItem.ID, poppedItems[0].ID, "获取的项目ID应该匹配")

	// 验证弹出后队列为空
	assert.Equal(t, 0, memQueue.Count(queueName), "弹出后队列应该为空")
}
...
func TestMemoryQueueDelayedItems(t *testing.T) {
	...
	err := memQueue.Add(queueName, delayedItem)
	assert.NoError(t, err, "添加延迟队列项应该成功")

	// 尝试获取项目，但应该不会返回（因为还没有到执行时间）
	assert.Empty(t, memQueue.Pop(queueName, 10), "不应该获取未到执行时间的项目")

	// 验证项目仍在队列中
	assert.Equal(t, 1, memQueue.Count(queueName), "延迟的项目应该仍在队列中")
...
func TestMemoryQueueDelete(t *testing.T) {
	...
	// 测试删除
	err = memQueue.Del(queueName, testItem.ID)
	assert.NoError(t, err, "删除队列项应该成功")
	assert.Equal(t, 0, memQueue.Count(queueName), "删除后队列应该为空")

	// 测试删除不存在的项目
	err = memQueue.Del(queueName, "non-existing-id")
...
func TestMemoryQueuePagination(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		...
		memQueue.Add(queueName, item)
	}

	assert.Equal(t, 20, memQueue.Count(queueName), "队列应该有20个项目")

	// 测试第一页
	page1 := memQueue.List(queueName, 1, 5)
	assert.Len(t, page1, 5, "第一页应该有5个项目")

	// 测试第二页
	page2 := memQueue.List(queueName, 2, 5)
	...
	// 这里我们只测试超出范围的页码
	assert.Empty(t, memQueue.List(queueName, 5, 5), "超出范围的页应该返回空列表")
}
